...
export function generatePRComment({ result }: ExportData): string {
  ...
  lines.push('');
  lines.push(`> Generated by [Ground Floor Check](https://ground-floor-scan.lovable.app). **${attentionCount} issue${attentionCount !== 1 ? 's' : ''} require${attentionCount === 1 ? 's' : ''} attention before merge.**`);
