    ...
    runs: [
      {
        tool: {
          driver: {
            name: 'Ground Floor Ethiscanner',
            version: '2.0.0',
            informationUri: 'https://ground-floor-scan.lovable.app',
            rules,
          },
        },
        results,
...
export function generatePRComment({ result }: ExportData): string {
  ...
  lines.push(`## 🛡️ Ground Floor Ethiscanner — Scan Results (GFS: ${gfs}/100)`);
  lines.push('');
  ...
  lines.push('');
  lines.push(`> Generated by [Ground Floor Ethiscanner](https://ground-floor-scan.lovable.app). **${attentionCount} issue${attentionCount !== 1 ? 's' : ''} require${attentionCount === 1 ? 's' : ''} attention before merge.**`);
